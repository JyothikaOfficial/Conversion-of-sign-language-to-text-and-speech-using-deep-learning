...
class Application:
    ...
    def __init__(self):
        ...
        self.pause_threshold = 3.0  # 1 second pause threshold
    ...
    def video_loop(self):
        try:
            ...
            if cv2image.any:
                ...
                if hands and hands[0]:  # Hand detected
                    self.last_hand_time = time.time()  # Update last hand detected time
                    self.space_added = False  # Reset space flag on hand detected
                    ...
                    # No hand detected: check pause duration
                    if time.time() - self.last_hand_time > self.pause_threshold:
    ...
    # def speak_fun(self):
    #     self.speak_engine.stop()   # Stop any current speech
    #     self.speak_engine = pyttsx3.init()  # Reinitialize engine
    #     self.speak_engine.setProperty("rate", 100)
    #     voices = self.speak_engine.getProperty("voices")
    #     self.speak_engine.setProperty("voice", voices[0].id)
    ...
    # def speak_fun(self):
    #     if self.str.strip():  # check if there's something to say
    #         self.speak_engine.stop()
    #         time.sleep(0.1)  # tiny delay to let engine reset
    #         self.speak_engine.say(self.str)
    #         self.speak_engine.runAndWait()
    def speak_fun(self):
        import pyttsx3  # just to be sure

        # Create a new engine instance every time
        engine = pyttsx3.init()
    ...
    # def speak_fun(self):
    #     self.speak_engine.stop()  # Stop any ongoing speech before starting a new one
    #     self.speak_engine.say(self.str)
    #     self.speak_engine.runAndWait()
